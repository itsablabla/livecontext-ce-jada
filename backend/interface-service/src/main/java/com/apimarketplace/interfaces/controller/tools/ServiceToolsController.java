...
/**
 * REST Controller for tool execution in interface-service.
 * Exposes the same /api/agent-tools/execute endpoint format as orchestrator/agent-service.
 * Routes directly to the local InterfaceToolsProvider (no HTTP hop).
...
public class ServiceToolsController {
    ...
    public ResponseEntity<Map<String, Object>> executeTool(
        ...
        copyIfPresent(request, "allowedTableIds", variables, "allowedTableIds");
