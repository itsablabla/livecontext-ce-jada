...
/**
 * Regression tests for the guardrail node ignoring model execution links.
 *
 * <p>Same defect as the classify node: the workflow guardrail node ran its own
 * {@code (provider, model)} pair unconditionally, so a linked model still consumed
 * the billed provider's own API key. Every test here fails on the pre-fix service.
 */
@DisplayName("GuardrailService - model execution links")
@ExtendWith(MockitoExtension.class)
class GuardrailServiceExecutionLinkTest {
    ...
    private static final List<GuardrailRequestDto.RuleDto> RULES = List.of(
        new GuardrailRequestDto.RuleDto("hate_speech", "No hate speech")
    );
    ...
    @BeforeEach
    void setUp() {
        service = new GuardrailService(agentLoopService, guardChainFactory, new ObjectMapper(),
    ...
    @Test
    @DisplayName("a link to a CLI bridge sends the check through the bridge in restricted mode")
    ...
    void apiLinkMovesTheLoopOnly() {
        ...
        GuardrailResponseDto result = service.execute(request());
    ...
    @Test
    @DisplayName("an UNLINKED bridge run is restricted too, so a guardrail check never gets a CLI's tools or cwd")
    ...
    private GuardrailRequestDto request() {
    ...
    private AgentLoopResult loopResultEchoing(String provider, String model) {
        UsageInfo usage = UsageInfo.builder().promptTokens(160).completionTokens(40).totalTokens(200).build();
