    ...
    public static List<String> getSuggestedActions(WorkflowBuilderSession session) {
        ...
        return switch (phase) {
            ...
            case BUILDING -> List.of(
                ...
                "add_node (utility: transform, wait, http_request, download_file, public_link, media, data_input, code, approval, exit, response)",
    ...
    public static String getActionHelp(String action) {
        return switch (action) {
            ...
            case "add_mcp" -> """
                ...
                ONE workflow, SEVERAL accounts of the same service (e.g. post to whichever
                Instagram account each row names): add credential_selector to params, set to an
                expression resolving to the account NAME. Under a split it varies per item, which
                is what makes one workflow serve many accounts:
                credential_selector: '{{item.ig_account}}'
                Get the names from get_connected_services (or credential(action='list'), whichever you have) and match
                one whose integration is this tool's; only a status of 'active' is selectable.
                Leave it out for every ordinary step: the step then runs on the owner's default
                account. Resolving to nothing FAILS the step, it never falls back to the default.
                """;
            case "add_agent" -> """
            ...
            case "add_generate", "generate" -> """
                ...
                the format produced, which other params are accepted and the price. Model ids cannot be guessed: workflow(action='help',
                topics=['generate']) lists every id this installation offers, with what each accepts and which param it is billed on.
                That is a free read and you always have it. The generation tool answers the same list with action='models' plus the
            ...
            case "add_sub_workflow", "sub_workflow" -> "workflow(action='add_node', type='sub_workflow', label='Run Child', params={workflow_id: '<uuid>'}, connect_after='...') - Execute another workflow as a sub-step. WAITS for that workflow to FINISH (up to timeoutSeconds, default 300, max 1500) and returns its step outputs; if it has not finished by then this node FAILS rather than returning a half-done result, and the target keeps running. A target that pauses on a user approval or an interface waiting for __continue can never finish on its own, so put those steps in the CALLING workflow instead. PRECONDITION: the child must ALREADY have a live run - this node never creates one. Run it once first with workflow(action='execute', id='<uuid>'), otherwise the node fails with \"No active run found\". If the child is pinned, only runs at its pinned version count.";
            ...
            case "add_send_email", "send_email" -> "workflow(action='add_node', type='send_email', label='Notify', params={toEmail: '{{trigger:form.output.email}}', subject: 'Update', body: '...'}, connect_after='...') - Send email via SMTP. SMTP credentials (host, port, password) are configured in Settings > Credentials - specify toEmail, subject, body, ccEmail, bccEmail, fromName, isHtml, replyTo (Reply-To header, when answers should reach a different address than the sender), fromEmail (sender override; leave it out to send as the credential's own address, which is what most SMTP servers accept). On failure the node returns success=false and sent=false, so {{core:<label>.output.success}} is a reliable guard. To REPLY in-thread, set inReplyTo to the original message's messageId (from email_inbox output) and toEmail to its from address.";
            case "add_email_inbox", "email_inbox" -> "workflow(action='add_node', type='email_inbox', label='Read Inbox', params={folder: 'INBOX', unreadOnly: true, limit: 20}, connect_after='...') - Read a mailbox via IMAP (IMAP credentials in Settings > Credentials). action='none' (default) READS → output.messages[] (each: uid, from, to, cc, subject, body, bodyHtml, hasAttachments, attachments[{filename,contentType,size,file?}]); filter with unreadOnly/flaggedOnly/sinceDays/beforeDays/fromContains/subjectContains/bodyContains, set downloadAttachments=true to save attachments to file storage. action='list_folders' → output.folders[]. action='create_folder' creates params.targetFolder → output.created (false when it already existed, so it is safe to re-run). action=mark_read/mark_unread/flag/unflag/move/delete acts on one message via params.messageUid (move needs targetFolder; createTargetIfMissing=true creates it instead of failing). ALWAYS run action='list_folders' FIRST and build targetFolder from a name in output.folders[]: folder names are SERVER PATHS, and some servers namespace everything under the inbox ('INBOX.Clients', separator '.') while others do not. create_folder and createTargetIfMissing create targetFolder EXACTLY AS WRITTEN and never correct it, so a guessed name silently creates a second, wrong folder and files mail into it instead of failing loudly. IMAP reads/acts only - to SEND or REPLY use send_email.";
            ...
            case "restart_from_node" -> "workflow(action='restart_from_node', run_id='<run>', node='<node-key>', epoch=<int>?) - Re-run ONE node of an existing run and continue from there, keeping the outputs the run already produced upstream. Use it instead of execute when only part of a run needs redoing. Omit epoch to replay the run's most recent fire; pass epoch=N (as get_run reports it) to repair an older one. Returns {outcome, replayed_nodes, epoch, attempt, status, summary} - read outcome before assuming the work finished, and epoch to confirm which fire ran.";
