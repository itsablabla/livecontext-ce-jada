...
public class ApplicationHelpModule implements ToolModule {
    ...
    private ToolExecutionResult executeHelp(Object topicsRaw) {
        ...
        if (all || topics.contains("troubleshooting")) result.put("troubleshooting", Map.of(
            ...
            "acquire_permission_denied_self_hosted_only", "If acquire returns PERMISSION_DENIED saying the app is Community Edition exclusive, the app uses a local CLI agent (`ce_exclusive_features` contains CLI_AGENT), which has no host on this deployment. This is TERMINAL: retrying, changing parameters, or re-calling with a different trigger cannot succeed, and no action available to you can install it. Say so plainly and suggest an app without `ce_exclusive`, or building the equivalent yourself starting from workflow(action='init', name='<name>').",
            "acquire_permission_denied_plan", "If acquire returns PERMISSION_DENIED naming a PLAN (the app uses embedding columns / vector search), the workspace can install it as soon as its plan includes that capability. This is NOT terminal, and it is not something you can change: do not retry, name the plan the message gives and tell the user that upgrading unlocks the install, or offer an app whose `ce_exclusive_features` does not list VECTOR_SEARCH.",
    ...
    private Map<String, Object> buildResponseFieldDocs() {
        ...
        r.put("workflowId", "UUID - the UNDERLYING workflow, NOT the application_id. Passing it as application_id to get/execute returns RESOURCE_NOT_FOUND (the error echoes the correct application_id). You may load it with workflow(action='load') to READ the plan, but an installed application's plan is frozen: every modifying action is refused with RESOURCE_CONFLICT / APPLICATION_PLAN_IMMUTABLE. Customising it is the user's move (they create an editable copy from the application page); you cannot create one.");
        r.put("ce_exclusive", "boolean - present (and always true) ONLY when the app needs something this deployment cannot provide AT ANY PLAN (today: a local CLI agent). ABSENT means installable here, including on a self-hosted install browsing the same catalogue - though an app whose `ce_exclusive_features` lists VECTOR_SEARCH can still need a higher plan, which acquire tells you by naming it. When present, acquire is REFUSED with PERMISSION_DENIED: do not call acquire, tell the user the app is self-hosted-only, and offer another app instead.");
        r.put("ce_exclusive_features", "array of strings - the special capabilities the app uses: 'CLI_AGENT' (a local CLI agent, installable on a self-hosted deployment only) and/or 'VECTOR_SEARCH' (embedding columns / similarity search, installable here from the plan an acquire refusal names). Present whenever one is detected, whether or not it blocks. Use it to explain in plain words what the app needs and whether an upgrade would be enough.");
        r.put("installed_resources", "object - returned by `acquire` when the install created resources BESIDES the application itself, as per-type counts, e.g. {interfaces: 2, tables: 1, agents: 1, workflows: 1}. Types that produced nothing are omitted, and the whole field is ABSENT when the application was the entire install. Tell the user what landed in their workspace instead of leaving them to find new rows in their interfaces / tables / agents lists. Counts what the clone really created, so it can differ from the numbers advertised on the marketplace card.");
        return r;
    }
    ...
    private Map<String, Object> buildActionDocs() {
        ...
        execution.put("execute", "Fire a trigger on an app you own or have acquired. Works DIRECTLY on `owned_by_me=true` apps (your own publications) - no acquire step. Returns same shape as workflow(action='execute') (run_id, epoch, status, all node statuses with output/error data, NEXT hint). Params: application_id (required), data_inputs (optional - use the names from `data_inputs_schema` returned by `get` or `create`, NOT from `my`/`search` which omit the schema, and do NOT guess), trigger_id (optional, defaults to `default_trigger_id`). Result also includes [visualize:application:<appId>:<runId>] marker that auto-renders an interactive preview card. INTERACTIVE CHAT: execute needs the user's authorization, and asking them happens inside your call - it may just take longer to answer while they decide. Do not stop, do not announce that you are waiting, and do not re-call: read what comes back. A normal run result (run_id, status, outputs) means they allowed it and it RAN; {executed:false} with status 'authorization_required', 'denied' or 'stopped' means the app did NOT run (no run_id, no outputs) - do not describe or invent results, do not call execute again, continue with other work or finish your turn. A response carrying run_id + status + outputs is the ONLY proof it actually ran.");
        execution.put("stop_run", "End a run that is still going - the counterpart of execute. Use it as soon as you can tell the execution went wrong (wrong input, a browser agent stuck on a login wall, a loop that will not converge) instead of waiting for it to finish. Params: run_id (from execute or runs), reason (optional but recommended - one sentence, recorded on the run and returned by get_run as stop_reason), mode (optional). Nodes still in flight are cancelled, including any browser-agent session, and no further node starts. CHOOSING THE MODE: mode='cancel' (the default) ends the run for good AND SUSPENDS THE SCHEDULES of the app's workflow, so a scheduled app stops firing until it is reactivated; mode='graceful' only closes the epoch that is running, returns the run to WAITING_TRIGGER and leaves the schedules alone - prefer it when you only want to end THIS execution. 'graceful' needs a run that is currently executing (a run parked on an approval, an interface or a timer still counts): it is refused when the run is PENDING or WAITING_TRIGGER, and the error tells you to use 'cancel'. Response: run_id, status, previous_status, mode, stopped_by, reason, plus self=true on a self-abort and already_terminal=true when the run had already ended (nothing is changed then, and no cause is recorded). A run that has not started executing yet cannot be stopped at all. SELF-ABORT: an agent running INSIDE a workflow can omit run_id to stop the run it is executing inside; it must then stop working and report the reason, because none of its further tool calls will run - and as a sub-agent that means the WHOLE parent run, not just its own step. Identical action and parameters on workflow(action='stop_run').");
    ...
    private Map<String, Object> buildParameterDocs() {
        ...
        params.put("action", "string, required - create|search|my|get|acquire|uninstall|execute|stop_run|runs|get_run|get_node_output|visualize|help");
