 ...
 *
 * This is the trigger behind the palette's lazy sections: a section that has never
 * been scrolled to must not fetch anything, and once it has been revealed it must not
 * un-reveal on the next scroll (that would refetch on every pass and, worse, unmount a
 * list the user is reading). Hence "once" - the latch never flips back.
 *
