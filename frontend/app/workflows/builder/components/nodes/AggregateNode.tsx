...
export function AggregateNode({ data, selected }: NodeProps<BuilderNodeData>) {
  ...
  const visuals = getNodeVisual('aggregate');
  ...
  // Step-by-step execution status for the aggregate node itself
  const stepByStepStatus = useNodeExecutionStatus(data.id, { label: data.label, kind: 'aggregate', status: data.status });
