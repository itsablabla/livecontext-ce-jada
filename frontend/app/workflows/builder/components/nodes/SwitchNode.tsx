...
export function SwitchNode({ data, selected, id }: NodeProps<BuilderNodeData>) {
  ...
  const executionStatus = useNodeExecutionStatus(id, { label: data.label, kind: 'switch', status: data.status });
    ...
    >
      ...
      <div className={`mt-4 ${getBranchRowFlow(layoutDirection)} text-[11px] text-slate-500`} style={
        ...
        {switchCases.map((caseRow, index) => {
          ...
          const handleId = caseRow.id;

          return (
            <div key={caseRow.id} className="relative rounded-2xl border border-theme px-3 py-2">
              <div className="flex items-center gap-2">
                <span className={clsx(
                  'text-[10px] font-semibold uppercase tracking-wide px-1.5 py-0.5 rounded',
                  caseRow.type === 'default'
                    ? 'bg-slate-200 dark:bg-slate-700 text-slate-600 dark:text-slate-300'
                    : 'bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300'
                )}>
                  {caseRow.type === 'default' ? 'DEFAULT' : 'CASE'}
                </span>
                <span className="truncate">{caseRow.label}</span>
              </div>
              <Handle
                type="source"
                id={handleId}
      ...
      {/* Status badge positioned at bottom right, above the last case */}
      ...
      {/* Step-by-step play button for switch nodes */}
