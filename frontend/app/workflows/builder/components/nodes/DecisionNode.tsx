...
export function DecisionNode({ data, selected, id }: NodeProps<BuilderNodeData>) {
  ...
  const visuals = getNodeVisual('decision');
  const conditions: ConditionRow[] =
    (data.decisionConditions as ConditionRow[] | undefined) ?? createDefaultDecisionConditions(data.id);
  ...
  const executionStatus = useNodeExecutionStatus(id, { label: data.label, kind: 'decision', status: data.status });
    ...
    >
      ...
      <div className={`mt-4 ${getBranchRowFlow(layoutDirection)} text-[11px] text-slate-500`} style={
        ...
        {conditions.map((condition, conditionIndex) => {
          ...
          // Always include index to ensure unique keys (handles duplicates and undefined IDs)
          const conditionKey = `${condition.id || id}-${conditionIndex}`;

          return (
            <div key={conditionKey} className="relative rounded-2xl border border-theme px-3 py-2">
      ...
      {/* Status badge positioned at bottom right, above the last condition */}
      ...
      {/* Step-by-step play button for decision nodes */}
